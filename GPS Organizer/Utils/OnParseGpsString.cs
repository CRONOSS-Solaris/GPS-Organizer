...
namespace GPS_Organizer.Utils
{
    public static class OnParseGpsString
    {
        public static ParsedGpsData ParseGpsString(string gpsString)
        {
            ...
            {
                Name = components[0],
                XCoord = double.Parse(components[1]),
                YCoord = double.Parse(components[2]),
                ZCoord = double.Parse(components[3]),
                Color = (Color)ColorConverter.ConvertFromString("#" + components[4])
            };
        }
